...
constexpr const wchar_t* VERSION = L"0.4.0";
...
// JS protocol) changes shape. apply_patch() skips on matching sentinel,
// so a no-bump update leaves stale code in the asar while the shim files
// on disk are fresh. v3 drops the extra.cfg / native-runtime gate.
constexpr int         SENTINEL_VERSION = 3;
